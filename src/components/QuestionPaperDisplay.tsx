...
interface QuestionPaperDisplayProps {
  paper: QuestionPaper;
  // FIX: Add onNewPaper and isMobile props to fix TypeScript error from App.tsx.
  onNewPaper: () => void;
  isMobile: boolean;
}
...
export const QuestionPaperDisplay: React.FC<QuestionPaperDisplayProps> = ({ paper, onNewPaper }) => {
  ...
  const handlePrint = () => {
    const printablePaperElement = document.getElementById('printable-paper');
    if (!printablePaperElement) return;

    // Conditionally add a class to the printable area to control answer visibility via CSS
    if (includeAnswersInExport) {
      printablePaperElement.classList.add('print-with-answers');
    }
    ...
    setTimeout(() => {
      printablePaperElement.classList.remove('print-with-answers');
    }, 100);
  };
  ...
  const handleDocxExport = () => {
    ...
    const sections = paper.sections.flatMap((section) => {
      const questions = section.questions.flatMap((q, qIndex) => {
        ...
        if (includeAnswersInExport) {
          questionParagraphs.push(new Paragraph({
            children: [
              new TextRun({ text: `Answer: ${q.correct_answer}`, bold: true, color: "008000" }),
            ],
            spacing: { after: 200 },
            indent: { left: 720 },
          }));
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-slate-200 print-container">
      <div className="p-6 border-b border-slate-200 flex flex-col md:flex-row justify-between items-center gap-4 no-print">
        <h2 className="text-xl font-bold text-slate-700">Generated Paper</h2>
        <div className="flex items-center gap-2 flex-wrap justify-center">
          <button
            onClick={onNewPaper}
            className="flex items-center gap-2 py-2 px-4 rounded-md text-sm font-semibold bg-green-600 text-white hover:bg-green-700 transition-colors"
          >
              <CreateIcon />
              New Paper
          </button>
          ...
          <div className="flex items-center pl-2 bg-slate-100 rounded-md">
            <input
              type="checkbox"
              id="include-answers"
            ...
            />
            <label htmlFor="include-answers" className="ml-2 text-sm text-slate-600 pr-3 py-2 select-none">
              Answers in Export
            </label>
          </div>
          <button
            ...
            <ol className="list-decimal list-inside space-y-6">
              {section.questions.map((q, qIndex) => (
                <li key={qIndex} className="break-words">
                  ...
                  {showAnswers && (
                    <div className="printable-answer mt-3 ml-6 p-2 bg-green-50 border border-green-200 rounded-md">
